...
# endif 
   ...
   SUBROUTINE isf_nam
      ...
      NAMELIST/namisf_drk/ nn_fwfisf_freq, sn_isfpar_fwf2
#endif
      !
...
#if defined key_drakkar
      !
      READ  ( numnam_ref, namisf_drk, IOSTAT = ios, ERR = 903)
903   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namisf_drk in reference namelist' )
      !
      READ  ( numnam_cfg, namisf_drk, IOSTAT = ios, ERR = 904 )
904   IF( ios >  0 )   CALL ctl_nam ( ios , 'namisf_drk in configuration namelist' )
      IF(lwm) WRITE ( numond, namisf_drk )
#endif
