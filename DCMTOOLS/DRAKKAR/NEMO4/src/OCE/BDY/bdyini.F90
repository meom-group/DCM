...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nambdy in configuration namelist', lwp )
   ...
   SUBROUTINE bdy_segs
      ...
      INTEGER  ::   iipt, ijpt, irpt                       !  local integers !JMM
#endif
      !!----------------------------------------------------------------------
      !
      cgrid = (/'t','u','v'/)
...
#if defined key_drakkar
      ! more logical writing but equivalent
      iwe = nimpp - 1 + 2         ! if monotasking and no zoom, iw=2
      ies = nimpp + nlci-1 - 1  ! if monotasking and no zoom, ie=jpim1
      iso = njmpp - 1 + 2         ! if monotasking and no zoom, is=2
      ino = njmpp + nlcj-1 - 1  ! if monotasking and no zoom, in=jpjm1
#else
...
#if defined key_drakkar
!    Drakkar rewrite of the code to make it easier to debug
!    bug found on 4 lines see below
         DO igrd = 1, jpbgrd
            icount  = 0
            ...
            DO ir=1, nn_rimwidth(ib_bdy)
               DO ib = 1, nblendta(igrd,ib_bdy)
                  ...
                  ! iipt, ijpt define the working boundary point, irpt the rim to which it belongs
                  ! iipt, ijpt refer to the global domain
                  iipt=nbidta(ib,igrd,ib_bdy)
                  ijpt=nbjdta(ib,igrd,ib_bdy)
                  irpt=nbrdta(ib,igrd,ib_bdy)

                  IF ( iipt >= iwe .AND. iipt <= ies .AND. ijpt >= iso .AND. ijpt <= ino .AND. irpt == ir ) THEN
                     !
                     icount = icount  + 1
...
!                     idx_bdy(ib_bdy)%nbj(icount,igrd)   = nbjdta(ib,igrd,ib_bdy)- mjg(1)+1
                     idx_bdy(ib_bdy)%nbi(icount,igrd)   = iipt - nimpp+1
                     idx_bdy(ib_bdy)%nbj(icount,igrd)   = ijpt - njmpp+1
                     ! check if point has to be sent
                     ii = idx_bdy(ib_bdy)%nbi(icount,igrd)
                     ij = idx_bdy(ib_bdy)%nbj(icount,igrd)
!!                     if((com_east .ne. 1) .and. (ii == (nlci-1)) .and. (nbondi .le. 0)) then   !! JMM   BUG FIX
                     if((com_east .ne. 1) .and. (ii == nlci) .and. (nbondi .le. 0)) then   !! JMM
                        com_east = 1
                     ...
                     if((com_south .ne. 1) .and. (ij == 2) .and. (nbondj .ge. 0) .and. (nbondj .ne. 2)) then
                        com_south = 1
!!                     elseif((com_north .ne. 1) .and. (ij == (nlcj-1)) .and. (nbondj .le. 0)) then !! JMM   BUG FIX
                     elseif((com_north .ne. 1) .and. (ij == nlcj) .and. (nbondj .le. 0)) then  !! JMM
                        com_north = 1
                     endif 
                     idx_bdy(ib_bdy)%nbr(icount,igrd)   = irpt
                  ...
                  SELECT CASE ( nbondi )
                  CASE ( 0 )
                     IF ( iipt >= iw_b(1) .AND. iipt <= ie_b(1) .AND. ijpt >= is_b(1) .AND.  ijpt  <= in_b(1) .AND. irpt == ir ) THEN
                       ii = iipt - iw_b(1)+2
!!                       if((com_west_b .ne. 1) .and. (ii == (nlcit(nowe+1)-1))) then  !JMM   BUG FIX
                     ...
                     ENDIF
                     IF ( iipt >= iw_b(2) .AND. iipt <= ie_b(2) .AND. ijpt >= is_b(2) .AND.  ijpt  <= in_b(2) .AND. irpt == ir ) THEN
                       ii = iipt - iw_b(2)+2
...
!======
                  SELECT CASE ( nbondj )
                  CASE ( 0 )
                     IF(com_north_b .ne. 1 .AND. ( iipt == iw_b(4)-1 .OR. iipt == ie_b(4)+1) .AND. ijpt == is_b(4) .AND. irpt == ir ) THEN
                       com_north_b = 1 
                     ENDIF
                     IF(com_south_b .ne. 1 .AND. ( iipt == iw_b(3)-1 .OR. iipt == ie_b(3)+1) .AND. ijpt == in_b(3) .AND. irpt == ir ) THEN
                       com_south_b = 1 
                     ENDIF
                     IF ( iipt >= iw_b(3) .AND.   iipt <= ie_b(3) .AND. ijpt >= is_b(3)    .AND. ijpt <= in_b(3) .AND. irpt == ir ) THEN
                       ij = ijpt - is_b(3)+2
!                       if((com_south_b .ne. 1) .and. (ij == (nlcjt(noso+1)-1))) then ! JMM   BUG FIX
                       if((com_south_b .ne. 1) .and. (ij == (nlcjt(noso+1)))) then
                          com_south_b = 1
                       endif 
                     ENDIF

                     IF ( iipt >= iw_b(4) .AND. iipt <= ie_b(4) .AND. ijpt >= is_b(4) .AND.  ijpt  <= in_b(4) .AND. irpt == ir ) THEN
                       ij = ijpt - is_b(4)+2
